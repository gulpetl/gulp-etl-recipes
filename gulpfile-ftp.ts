...
export function downloadFTP(callback: any) {
    try {    
        return conn.src(['Tap/*'], {base:'/Tap/'})     //Read files from Tap folder in FTP Server; Base takes all the files inside folder
...
export function uploadFTP(callback: any) {
        try {    
            return src(['testdata/csv/*.csv'], {base:'testdata/csv/'}) //Grabs all files in local folder named "data"; Base takes all those files
            .pipe(errorHandler(function(err:any) {
                console.error('oops: ' + err)
                callback(err)
              })) 
